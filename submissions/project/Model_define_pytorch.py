...
def get_efficientnet(model_name='efficientnet-b0', pretrained=True):
    if pretrained:
...
def get_resnet(modelchoice='resnet50', pretrained=True):
...
def get_vgg(modelchoice='vgg16', pretrained=True):
...
class Encoder(nn.Module):
    ...
    def __init__(self, feedback_bits, efn_name='efficientnet-b0'):
        ...
        # TODO 提交代码时候的 pretrained 一定要设置为False
        if 'ns' in efn_name:
            self.efn, self.feat_dim = get_efficientnet_ns(model_name=efn_name, pretrained=False)
            self.efn.conv_stem.in_channels = 2
            self.efn.conv_stem.weight = torch.nn.Parameter(self.efn.conv_stem.weight[:, 0:2:, :, :])
        elif 'efficientnet' in efn_name:
            self.efn, self.feat_dim = get_efficientnet(model_name=efn_name, pretrained=False)
            self.efn._conv_stem.in_channels = 2
            self.efn._conv_stem.weight = torch.nn.Parameter(self.efn._conv_stem.weight[:, 0:2:, :, :])
        elif 'res' in efn_name:
            self.efn, self.feat_dim = get_resnet(modelchoice=efn_name, pretrained=False)
            self.efn.conv1.in_channels = 2
            self.efn.conv1.weight = torch.nn.Parameter(self.efn.conv1.weight[:, 0:2:, :, :])
        elif 'vgg' in efn_name:
            self.efn, self.feat_dim = get_vgg(modelchoice=efn_name, pretrained=False)
            self.efn.features[0].in_channels = 2
            self.efn.features[0].weight = torch.nn.Parameter(self.efn.features[0].weight[:, 0:2:, :, :])
    ...
    def forward(self, x):
        ...
        elif 'efficientnet' in self.efn_name:
